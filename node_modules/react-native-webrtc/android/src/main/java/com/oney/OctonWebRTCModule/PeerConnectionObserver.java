package com.oney.OctonWebRTCModule;
...
import org.octonwebrtc.StatsObserver;
import org.octonwebrtc.StatsReport;
import org.octonwebrtc.VideoTrack;
import org.octonwebrtc.OctonFrameDecryptor;
import org.octonwebrtc.OctonFrameEncryptor;

class PeerConnectionObserver implements PeerConnection.Observer {
    private final static String TAG = WebRTCModule.TAG;

    private final SparseArray<DataChannel> dataChannels = new SparseArray<DataChannel>();
    ...
    boolean addStream(MediaStream localStream) {
        if (peerConnection != null && peerConnection.addStream(localStream)) {
            ...
            Log.d(TAG, "add local stream: ");
            int[] myIntArray = { 97, 145, 133, 123, 63, 197, 49, 232, 87, 159, 169,
                    200, 59, 123, 77, 75, 150, 173, 200, 232, 44, 39,
                    8, 149, 250, 6, 238, 170, 255, 17, 200, 12 };
            OctonFrameEncryptor fe1 = new OctonFrameEncryptor(myIntArray);
            peerConnection.setFrameEncryptor(fe1, myIntArray);
            return true;
        }
     ...
     *         this instance resulted in a modification of its internal list of
     *         local
    ...
    void close() {
        ...
        // by the PeerConnection instance (RtpReceivers, rtpRtpSenders, etc.)
    ...
    public void onIceCandidate(final IceCandidate candidate) {
        ...
        if (peerConnection != null) {
            int[] myIntArray = { 97, 145, 133, 123, 63, 197, 49, 232, 87, 159, 169,
                    200, 59, 123, 77, 75, 150, 173, 200, 232, 44, 39,
                    8, 149, 250, 6, 238, 170, 255, 17, 200, 12 };
            OctonFrameDecryptor de1 = new OctonFrameDecryptor(myIntArray);
            peerConnection.setFrameDecryptor(de1, myIntArray);
        }
    ...
    public void onAddStream(MediaStream mediaStream) {
        ...
        for (int i = 0; i < mediaStream.videoTracks.size(); i++) {
            Log.d(TAG, "Ivan onAddStream: video track " + i);
        ...
        for (int i = 0; i < mediaStream.audioTracks.size(); i++) {
            Log.d(TAG, "Ivan onAddStream: audio track " + i);
    ...
    private String peerConnectionStateString(PeerConnection.PeerConnectionState peerConnectionState) {
        switch (peerConnectionState) {
            ...
            case CONNECTED:
                // Added by Ivan
                Log.d(TAG, "Ivan peerConnectionStateString connected");
